...
class ChanelWiseFC(nn.Module):  # 逐通道全连接
    ...
    def forward(self, x):
        y = torch.split(x, 1, dim=1)  # 在通道维度分离
        list = []
        for i in y:
        ...
        z = torch.stack(list, dim=1)  # 把前面分裂后的y在通道维度拼接
        return z
...
class GroupWiseDeformableConv(nn.Module):  # 分组可变形卷积
    def __init__(self, channels, num_group, kernel_size=3, stride=1, padding=1):
    ...
    def forward(self, x):
        y = torch.split(x, x.shape[1] // self.k, dim=1)  # 在通道维度上分离出k组
        list = []
        for i in y:
...
class Atten3D(nn.Module):  # 3D Atten模块,(创新点1)
    def __init__(self, in_planes, inSize, outSize, stage, en_defor=False, num_group=2):  # 输入通道数，fc输入尺寸，fc输出,阶段(对应resnet的stage)
        ...
        # 下面是可变形组卷积的实现
        self.groupWiseDeformableConv = GroupWiseDeformableConv(in_planes, num_group, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        if self.en_defor and (self.stage == 4 or self.stage == 5):
