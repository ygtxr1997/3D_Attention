...
int deform_conv_forward_cuda(THCudaTensor *input, THCudaTensor *weight,
  ...
  int batch = 1;
  if (THCudaTensor_nDimension(state, input) == 3) {// 若输入是三维的，则增加一维来保证计算的一致性，增至4维(在第一维插入)
    // Force batch
    batch = 0;
    THCudaTensor_resize4d(state, input, 1, THCudaTensor_size(state, input, 0), THCudaTensor_size(state, input, 1),
                          THCudaTensor_size(state, input, 2));//对input
    THCudaTensor_resize4d(state, offset, 1, THCudaTensor_size(state, offset, 0), THCudaTensor_size(state, offset, 1),
                          THCudaTensor_size(state, offset, 2));//对offset
  }

  // todo: assert batchsize dividable by im2col_step

  long batchSize = THCudaTensor_size(state, input, 0);//张量的四维，理解THCudaTensor_size()是获取张量的大小
  ...
  long nOutputPlane = THCudaTensor_size(state, weight, 0);//输出的大小，应该指的是通道，因为这个output没有添加batch

  long outputWidth = (inputWidth + 2 * padW - (dilationW * (kW - 1) + 1)) / dW + 1;//输出的宽和高
  long outputHeight = (inputHeight + 2 * padH - (dilationH * (kH - 1) + 1)) / dH + 1;

  THArgCheck((THCudaTensor_size(state, offset, 0) == batchSize), 3, "invalid batch size of offset");//检测offset的batch
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {// 在每一个组内进行计算
    ...
    deformable_im2col( // 构造columns向量，每一列代表与每个卷积核作用的元素
